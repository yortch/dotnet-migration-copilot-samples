...
namespace ContosoUniversityCore.Controllers
{
    public class CoursesController : BaseController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (teachingMaterialImage != null && teachingMaterialImage.Length > 0)
                {
                    ...
                    try
                    {
                        ...
                        var uploadsPath = GetTeachingMaterialsPath();
            ...
            if (ModelState.IsValid)
            {
                ...
                if (teachingMaterialImage != null && teachingMaterialImage.Length > 0)
                {
                    ...
                    try
                    {
                        ...
                        {
                            var oldFilePath = MapRelativeUrlToFilePath(course.TeachingMaterialImagePath);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            if (!string.IsNullOrEmpty(course.TeachingMaterialImagePath))
            {
                var filePath = MapRelativeUrlToFilePath(course.TeachingMaterialImagePath);
        ...
        private string GetTeachingMaterialsPath()
        {
            return Path.Combine(WebHostEnvironment.WebRootPath, "Uploads", "TeachingMaterials");
        }

        private string MapRelativeUrlToFilePath(string relativeUrl)
        {
            var normalizedPath = relativeUrl.TrimStart('~', '/').Replace('/', Path.DirectorySeparatorChar);
            return Path.Combine(WebHostEnvironment.WebRootPath, normalizedPath);
        }
