...
namespace ContosoUniversity.Controllers
{
    public class CoursesController : BaseController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (teachingMaterialImage != null && teachingMaterialImage.Length > 0)
                {
                    ...
                    try
                    {
                        ...
                        {
                            var oldFilePath = Path.Combine(_webHostEnvironment.ContentRootPath, "wwwroot",
                                course.TeachingMaterialImagePath.Replace("~/", "").Replace('/', Path.DirectorySeparatorChar));
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            if (!string.IsNullOrEmpty(course.TeachingMaterialImagePath))
            {
                var filePath = Path.Combine(_webHostEnvironment.ContentRootPath, "wwwroot",
                    course.TeachingMaterialImagePath.Replace("~/", "").Replace('/', Path.DirectorySeparatorChar));
